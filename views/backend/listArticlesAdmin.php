   ...
   <div class=".col-md-3">
      <div class="card mb-3 shadow-sm">
         <svg class="bd-placeholder-img card-img-top" width="100%" height="75" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid slice" focusable="false" role="img" aria-label="Placeholder: Thumbnail">
            <title>liste articles Admin</title>
            <rect width="100%" height="100%" fill="#55595c"/>
            <text x="50%" y="50%" fill="#eceeef" dy=".3em"><?=($data['title']) ?></text>
         </svg>
         <div class="card-body">
            <p class="card-text" align="center">Catégorie : <?=($data['libele']) ?></p>
...
<div align="center">
   <?php // touche Précédente
      if($pageCourante > 1): ?>
   <a href="index.php?action=listArticlesAdmin&amp;page=<?php if($pageCourante !='1'): echo $pageCourante -1; ?><?php else: echo $pageCourante; endif ?>" >Précédent</a>&nbsp;&nbsp;
   <?php endif ?>
   <?php
      for($i=1;$i<=$totalpages;$i++) : // boucle pour passer de pages en pages
         if($i == $pageCourante) :
          echo $i.'&nbsp;&nbsp;'; ?> <!--suppression du lien page courante-->
   <?php else : ?> <!-- liens aux numéros de page-->
   <?php echo '<a href="index.php?action=listArticlesAdmin&amp;page='.$i.'"> '.$i.'</a>&nbsp;&nbsp;';
      endif ?>
   <?php endfor ?>
   <?php // touche Suivante
      if($pageCourante < $totalpages): ?>
   <a href="index.php?action=listArticlesAdmin&amp;page=<?php if($pageCourante != $totalpages): echo $pageCourante +1; ?><?php else: echo $pageCourante; endif ?>" >Suivant</a>
   <?php endif ?>
</div>
<br>
